...
trait NonSuspendableTapeArchiveModule[F[_]] extends TapeArchiveModule[F] {
  ...
  override final def readTapeArchiveStreams(file: File): Enumerator[F, (String, Array[Byte])] =
  ...
  final class TapeArchiveEnumerator(ti: TarInputStream) extends Enumerator[F, (String, Array[Byte])] {
    final def apply[A](s: Step[F, (String, Array[Byte]), A]): F[Step[F, (String, Array[Byte]), A]] =
      F.tailRecM(s) { step =>
        if (step.isDone) F.pure(Right(step)) else F.flatten(
          F.catchNonFatal {
            ...
            if (nextEntry != null) {
              ...
              if (bytesRead == -1) F.pure(Right(step)) else F.map(step.feedEl((nextEntry.getName, read)))(Left(_))
